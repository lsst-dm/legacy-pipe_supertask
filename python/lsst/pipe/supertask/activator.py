...
class SuperTaskFactory(object):
    """Base class for activator classes which defines interface for
    instantiating super-tasks.
    """

    __metaclass__ = abc.ABCMeta

    def makeTaskInstance(self, taskName, config, overrides, butler):
        """Create new SuperTask instance.
        ...
        """
        # get object responsible for importing task class
        loader = self._getTaskLoader()

        # load the class
        taskClass, fullTaskName, taskKind = loader.loadTaskClass(taskName)
    ...
    @abc.abstractmethod
    def _getTaskLoader(self):
        """Return task loader instance.
